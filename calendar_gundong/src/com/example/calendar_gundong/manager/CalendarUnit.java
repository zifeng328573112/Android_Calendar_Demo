package com.example.calendar_gundong.manager;
...
public abstract class CalendarUnit {
	...
	protected CalendarUnit(@NonNull LocalDate from, @NonNull LocalDate to, @NonNull String headerPattern, @NonNull LocalDate today) {
		mToday = today;
		mFrom = from;
		mTo = to;
	...
	public boolean isInView(@NonNull LocalDate date) {
		return !mFrom.withDayOfWeek(DateTimeConstants.MONDAY).isAfter(date) && !mTo.withDayOfWeek(DateTimeConstants.SUNDAY).isBefore(date);
	}
	...
	public boolean equals(Object o) {
		...
		if (mSelected != that.mSelected)
			return false;
		if (!mFrom.equals(that.mFrom))
			return false;
		if (!mHeaderFormat.equals(that.mHeaderFormat))
			return false;
		if (!mTo.equals(that.mTo))
			return false;
		if (!mToday.equals(that.mToday))
			return false;
